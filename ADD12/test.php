      ...
      foreach($result as $row)
      {
            ...
            <div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:16px; height:450px;" align="center">
                  ...
                  <h4 class="text-danger">$ '.$row["price"] .'</h4>
                   <h4 class="text-danger">$ '.$row["brand"] .'</h4>
                  <input type="text" name="quantity" id="quantity' . $row["id"] .'" class="form-control" value="1" />
                  <input type="hidden" name="hidden_name" id="name'.$row["id"].'" value="'.$row["name"].'" />
                  <input type="hidden" name="hidden_price" id="price'.$row["id"].'" value="'.$row["price"].'" />
                  <input type="hidden" name="hidden_price" id="brand'.$row["id"].'" value="'.$row["brand"].'" />
...
?>
<input type="button" name="add_to_cart" id="'.$row["id"].'" style="float:right;margin-top0px;" class="btn btn-danger form-control add_to_cart" value="Add to Cart" />


<button id="'.$row["id"].'" style="float:right;" class="btn btn-danger btn-xs">Add to cart</button>